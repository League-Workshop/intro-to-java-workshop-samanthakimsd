...
public class RobotTreasureHunt implements KeyEventDispatcher {
	...
	private void spaceBarWasPressed() {
		...
		int robotXLocation = icee.getX();
		int robotYLocation = icee.getY();
		...
		if (Math.abs(robotXLocation - 730) < 10 && Math.abs(robotYLocation - 430) < 10) {
			JOptionPane.showMessageDialog(null, "Next go to boy");
		}
	}
	// 8. Give the user subsequent clues at different locations on the image
	// (pirate robot, swamp, parrots, etc.)
	if(Math.abs(robotXLocation-205)<10&&Math.abs(robotYLocation-425)<10){
		JOptionPane.showMessageDialog(null, "You found me, to find the next clue find the bottle in the swamp!");
